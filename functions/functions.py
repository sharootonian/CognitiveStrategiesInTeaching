# functions for TowerOfTerror task
# Sevan Harootonian 12/2021
...
# def to_policy(mdp,**kwargs):
#     # create a tot mdp with kwargs parameters
#     mdp = to_mdp(mdp,**kwargs)
#     # run policyiteration to find opt policy for this mdp
#     pi_result = PolicyIteration().plan_on(mdp)
#     return pi_result.policy


# # for IRL

# def create_condition_on_state_action_traj_function(mdp,state_traj, action_traj):
#     # create condition function that conditions on state and action traj
#     def condition_on_state_action_traj(rewardDict):
#         #find opt policy for rewardDict
#         sim_policy = to_policy(mdp,goal_values=rewardDict)

#         #loop through the s and a pairs from the observed state_traj and action_traj
#         for s, a in zip(state_traj, action_traj):

#             # look at all actions at state s according to the opt policy sim_policy
#             sim_action_dist = sim_policy[s]

#             # if this opt policy include a at state s then cont if not then this rewardDist will get filtered out (removed from posterior)
#             if a in sim_action_dist.support:
#                 continue
#             return False
#         return True

#     return condition_on_state_action_traj
...
# def create_trajectoryMatch(default_parameters, mentee_traj):
#     # create a function that computes optimum_traj and return a bool if it matchs with the mentee_traj
#     def trajectoryMatch(world):
#         # create mdp with the new world
#         # tot_temp = to_mdp(default_parameters,connections=world)
#         tot_temp_params = {**default_parameters, "connections": world}
#         tot_temp = create_mdp(frozendict(tot_temp_params))
        
#         # create find optimum policy
#         result_temp = ValueIteration().plan_on(mdp=tot_temp)
        
# #         # get trajectory of opt policy
# #         optimum_traj = result_temp.policy.run_on(tot_temp)
        
# #         return optimum_traj.state_traj == mentee_traj

#         # check whether trajectory has non-zero probability of being taken
#         for s, ns in zip(mentee_traj, mentee_traj[1:]):
#             if result_temp.policy.action_dist(s).prob(ns) == 0.0:
#                 return False
#         return True
