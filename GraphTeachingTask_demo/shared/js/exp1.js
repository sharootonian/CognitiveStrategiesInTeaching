  ...
  var jsPsych = initJsPsych({
    ...
    // const csvFile = "exampleInput.csv"
    const csvFile = "../../shared/data/exampleInput.csv"
    ...
    // HERE WE DO SOME PRACTICE TRAINING:
    ...
    var quiz = {
        ...
        questions: [
          ...
          },
          {
            ...
            name: 'q3', 
            options: ['Possible paths the student can take.', 'Possible paths you, as the teacher, can take.','Paths that the student took before.'], 
            required: true,
            horizontal: false
          },
          {
        ...
        on_finish: function() {
            ...
            if (jsPsych.data.getLastTrialData().values()[0].response.q3 != 'Possible paths the student can take.') {
              incorrect = true;
            }
    ...
    // makes graph trial
    // inter-trial instructions page
    // var betweenInstructions = {
    //     type: jsPsychHtmlKeyboardResponse,
    //     stimulus: '<p></p>',
    //     choices: "NO_KEYS",
    //     trial_duration: 500,
    ...
    function getCurrentEdge() {
      current_correct_edge=JSON.parse(localStorage.getItem("correctResponses"))[sessionStorage.indexInput-1]
      return current_correct_edge
    }
    function getFlipped() {
    ...
    function getModelSeed() {
      model_seed=JSON.parse(localStorage.getItem("seed"))[sessionStorage.indexInput-1]
      return model_seed
    }
    function getEdgeRes(){
    ...
    var debrief = {
      ...
      on_finish: function() {
          ...
          for (let i = 0; i < sessionStorage.indexInput; i++) {
            edges_selected[i]=sessionStorage.getItem("edgeSelection_" + i)
            bonus[i] = 0
            if(edges_selected[i]==JSON.parse(localStorage.getItem("correctResponses"))[i]){
              bonus[i] = 1
            }
          }
          data[0] = JSON.parse(localStorage.getItem("shuffledIndices"))
          data[1] = edges_selected
          data[2] = JSON.parse(localStorage.getItem("correctResponses"))
          data[3] = bonus

          sumbonus= data[3].reduce((a, b) => a + b, 0)*0.15
          localStorage.setItem("sumbonus", parseFloat((sumbonus).toFixed(2)))
          }
    };
