  ...
  var jsPsych = initJsPsych({
    ...
    var instructions = {
      type: jsPsychInstructions,
        ...
        '<p style="text-align: left; width: 800px; border: 1px solid white; margin: 0 auto;">\
          ...
          <br>\<br>\
          They will always start at the top circle and move downwards to one of the circles at the end. Each circle gives some # of points and the dashed lines are called "edges" and are possible paths the student can take. \
          <br>\<br>\
          Note: the student can never move upwards and must always go down or diagonally down.</p>'+
        '<img src="../../shared/img/inst_0_labeled.png" width="350"> </img>'+'<p>let\'s look step by step at how the student navigates with this new information.</p>'+'<p>press next</p>', 
        //page 4
        ...
        '<p style="text-align: left; width: 800px; border: 1px solid white; margin: 0 auto;">\
          <b>Note:</b> The student will always take the path that give the most total points. The student didn\’t know any edges that connected to the circle with the +3 points, so that\’s why they didn\’t go there.</p>'+ '<p>press next</p>',
        //page 9 teacher
        '<p style="text-align: left; width: 800px; border: 1px solid white; margin: 0 auto;">\
          Below is how you, as the <b>teacher</b>, will see the graph. <u>You know all the possible edges.</u> You are able to see which edges the student navigated and how many points they received. <br><br>\
          <u>You don\’t know which other edges the student knows or doesn\'t know, but you can assume that the student took the best path, given what they knew.</u></p>'+
          '<img src="../../shared/img/inst_3_teacher.png" width="350"></img>',
        //page 10
        '<p style="text-align: left; width: 800px; border: 1px solid white; margin: 0 auto;">\
          Now you are going to reveal a single edge to the student,<b>to improve the student\'s points the next time they navigate. </b>\
          <br>\<br>\
        ...
        '<img src="../../shared/img/inst_advice_3.png" width="350"></img>'+
        '<p>The student is done navigating and earned <b>6 points</b>.</p>'+
        '<p>The edge you chose to reveal was a good choice as it increased the student\'s points from 4 to 6! </p>'+
        '<p>press next</p>',
        // page 16
        '<p style="text-align: left; width: 800px; border: 1px solid white; margin: 0 auto;">\
          Now let\'s imagine you choose to reveal a different edge to the student (highlighted in yellow).</p>'+
        ...
        '<p>The student is done navigating and earned <b>4 points</b>.</p>'+
        '<p>The edge we chose to reveal was a <b>bad</b> choice as it didn\'t increase the student\'s points. </p>'+
        '<p>press next</p>',
        //page 21
        '<h3>Hopefully, you are getting the hang of it. Let\'s have you actually practice on a few students.</h3>'+
        '<h3>Note: Each of the students you help will know different possible edges. So each student is unique.</h3>',
        //page 22
        ...
        on_finish: function() {
              ...
              jsPsych.setProgressBar(curr_progress_bar_value + (10/(Number(ntrials)+20)));
          }
    ...
    var trainingInteractive = {
        ...
        // url: "./nodeGraphs_scaffolding.html",
        url: "./nodeGraphsTraining_scaffolding.html",
        cont_btn: "next_trial",
        ...
        seed : getModelSeed,
        trial_group:getTrial_group,
        congruency: getCongruency,
        condition: getCondition,
        trial_id: getTrial_id,
        inference_selectedEdgeIDs: getinference_SelectedEdgeIDs,
        inference_responsesLog: getinference_ResponsesLog,
        inference_currEdgeRTMS: getinference_CurrEdgeRTMS,
        inference_edge_res: getinference_EdgeRes,
        ...
        on_finish: function() {
            ...
            // based on the current value and the proportion to update for each trial
            var curr_progress_bar_value = jsPsych.getProgressBarCompleted();
            jsPsych.setProgressBar(curr_progress_bar_value + (5/(Number(ntrials)+20)));
        }
    };
    ...
    var quiz = {
        ...
        questions: [
          ...
          {
            prompt: "There is always one best edge to reveal?", 
            name: 'q2', 
            options: ['True', 'False'], 
            required: true,
            horizontal: false
          ...
          {
            prompt: "What do the dotted lines represent when selecting an edge?", 
            name: 'q3', 
            options: ['Possible edges you, as the teacher, can choose to teach the learner.', 'Possible edges you, as the teacher, can navigate.','Edges that the student took before.'], 
            required: true,
            horizontal: false
          },
          {
            prompt: "What do the solid black lines represent when selecting an edge?", 
            name: 'q4', 
            options: ['Possible edges you, as the teacher, can choose to teach the learner.', 'Possible edges you, as the teacher, can navigate.','Edges that the student took before.'], 
            required: true,
            horizontal: false
        ...
        on_finish: function() {
            ...
            if (jsPsych.data.getLastTrialData().values()[0].response.q3 != 'Possible edges you, as the teacher, can choose to teach the learner.') {
              incorrect = true;
            }
            if (jsPsych.data.getLastTrialData().values()[0].response.q4 != 'Edges that the student took before.') {
              incorrect = true;
            }
            if (incorrect == true) {
            quiz_fail=1
    ...
    var experiment_reminder = {
      ...
      var inference_edgeSelection = [];
      var inference_responsesLog = [];
      var inference_currEdgeRTMS = undefined;
      var inference_selectedEdgeIDs = [];
      var response_received = function(elem) {
          if (document.getElementById("selected").checked) {
            ...
            selectedEdgeIDs = (sessionStorage.getItem("selectedEdgeIDs"))
            skipped = (sessionStorage.getItem("skipped"))
            inference_responsesLog = (sessionStorage.getItem("inference_responsesLog"))
            inference_currEdgeRTMS = (sessionStorage.getItem("inference_edgeRTMS"))
            inference_selectedEdgeIDs = (sessionStorage.getItem("inference_selectedEdgeIDs"))
            inference_edgeSelection.push(sessionStorage.getItem("inference_edgeSelection"))
            sessionStorage.setItem("inference_edgeSelection","")
            return true;
          }
          else {
            alert("you must select an edge first.");
            return false;
          }
          return false;
      };
    ...
    function getCurrentEdge() {
      // Exp4 doesn't use correct edges, return null
      return null;
    }
    function getFlipped() {
    ...
    function getModelSeed() {
      // Exp4 doesn't use model seeds, return null
      return null;
    }
    ...
    function getTrial_group() {
      current_trial_group =JSON.parse(localStorage.getItem("trial_group"))[sessionStorage.indexInput-1]
      return current_trial_group
    }

    function getCondition() {
      current_condition =JSON.parse(localStorage.getItem("condition"))[sessionStorage.indexInput-1]
    ...
    function getinference_EdgeRes(){
      return inference_edgeSelection[inference_edgeSelection.length - 1];
    }

    function getinference_ResponsesLog(){
      return inference_responsesLog;
    }
    function getinference_CurrEdgeRTMS(){
      return inference_currEdgeRTMS;
    }

    function getinference_SelectedEdgeIDs(){
      return inference_selectedEdgeIDs;
    }
    ...
    var experiment = {
        ...
        on_finish: function() {
            ...
            // based on the current value and the proportion to update for each trial
            var curr_progress_bar_value = jsPsych.getProgressBarCompleted();
            jsPsych.setProgressBar(curr_progress_bar_value + (1/(Number(ntrials)+20)));
        }
    };
    var ntrials = sessionStorage.getItem("ntrials")
